...
const Icons = {
  ...
  Tree: () => (
    ...
    >
      ...
      <path d="M8 12c-2.5 0-4-1.5-4-1.5s1-2.5 3-2.5" />
      <path d="M16 12c2.5 0 4-1.5 4-1.5s-1-2.5-3-2.5" />
      <path d="M4 22h16" />
    </svg>
  ),
  Phone: () => (
...
export default function App() {
  ...
  const handlePhoneLogin = async () => {
    if (phoneInput.length < 10 || nameInput.length < 2)
      return alert("Please enter valid name and 10-digit number.");
  ...
  const connectToChaupal = async () => {
    ...
    // If user entered a phone number (more than 5 digits)
    if (targetId.length > 5) {
      const q = query(
        collection(db, "users"),
        where("phoneNumber", "==", targetId)
      );
      const snap = await getDocs(q);
  ...
  const answerCall = async () => {
    try {
      ...
      alert("Failed to answer. Check permissions.");
      setCallState(null);
    }
  };
      ...
      {callState && (
        <div className="call-overlay">
          ...
          <div className="call-controls">
            ...
            {callState === "calling" && (
              <p style={{ color: "#fff" }}>Calling...</p>
            )}
            <button onClick={endCall} className="btn-endcall">
              <Icons.PhoneOff />
            </button>
          </div>
        </div>
      ...
      {!activeRoom ? (
        ...
        >
          <header className="main-header">
            <h2
              style={{ color: "#D4A373", margin: 0, fontFamily: "Yatra One" }}
            >
              {activeTab === "chat" ? "Chaupals" : "Profile"}
            </h2>
            <Icons.MenuDots />
          </header>
          ...
          <main className="main-content" style={{ padding: "20px" }}>
            {activeTab === "chat" ? (
              <div className="dashboard-card animate-pop">
                ...
                <input
                  type="text"
                  placeholder="Enter Friend's 5-Digit ID or Mobile No."
                  value={connectInput}
                  onChange={(e) => setConnectInput(e.target.value)}
                  className="input-field"
                />
                <button
                  onClick={connectToChaupal}
                  className="btn-primary"
                  style={{ width: "100%" }}
                >
                  Start Chat
              ...
              <div className="profile-section animate-pop">
                ...
                <h3 style={{ color: "#EAE0D5", margin: "10px 0 0 0" }}>
                  {user.displayName}
                </h3>
                <p
                  style={{
                ...
                >
                  {user.phoneNumber || "Google Account"}
                </p>

                <div className="settings-list">
                  <div className="setting-item">
                    <span>Language</span>{" "}
                    <span style={{ color: "#D4A373" }}>English</span>
                  </div>
                  <div className="setting-item">
                    <span>Status</span>{" "}
        ...
        >
          <header className="chat-header">
            <button onClick={() => setActiveRoom(null)} className="icon-btn">
              <Icons.Back />
            </button>
            <div style={{ flex: 1, marginLeft: "15px" }}>
              <h2 style={{ color: "#EAE0D5", margin: 0, fontSize: "18px" }}>
                ID: {currentFriendId}
              </h2>
            ...
            {showMenu && (
              <div className="menu-dropdown">
                <button onClick={() => startCall(false)}>
                  <span style={{ marginRight: "8px" }}>📞</span> Voice Call
                </button>
                <button onClick={() => startCall(true)}>
                  <span style={{ marginRight: "8px" }}>📹</span> Video Call
                </button>
              </div>
            )}
          </header>
          ...
          <main className="chat-area">
            {messages.map((msg) => (
              <div
                key={msg.id}
                className={`message-bubble animate-pop ${
                  msg.senderId === setuId ? "sent" : "received"
                }`}
              >
                {msg.senderId !== setuId && (
          ...
          <div className="chat-input-area">
            ...
            <input
              ...
              onKeyDown={(e) =>
                e.key === "Enter" && sendMessage(inputText, "text")
              }
